...
export class CreateJobApplicationUsecase implements Usecase {
    public async execute(params: CreateJobApplicationParams): Promise<Result> {
        ...
        if (job.limitDate < new Date()) {
            return UsecaseResponse.invalidField("Deadline", "Job is not accepting applications anymore");
        }
        ...
        if (job.maxCandidates && job.maxCandidates <= applications.length) {
            return UsecaseResponse.invalidField("Job", "Job applications already fullfiled");
        }

        // 6- O candidato já se aplicou a mesma vaga
        if (applications.some((application) => application.candidate.id === user.id)) {
            return UsecaseResponse.invalidField("Candidate", "Candidate already subscribed");
        }
        ...
        return UsecaseResponse.success("Job Application successfully created", jobApplication);
    }
}
